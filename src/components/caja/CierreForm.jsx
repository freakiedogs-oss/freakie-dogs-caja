  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      Promise.all([
        ...
        db.from('depositos_bancarios').select('id,monto,monto_esperado,diferencia_deposito,estado,fotos_urls,notas').eq('store_code', existingCierre.store_code).neq('estado', 'anulado').contains('dias_cubiertos', [existingCierre.fecha]).limit(1),
      ]).then(([eg, ing, aj, dep]) => {
