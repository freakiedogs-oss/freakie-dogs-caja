...
function ListadoDTEs({ user }) {
  ...
  const [filtroInvalidado, setFiltroInvalidado] = useState('vigentes')
  const [expandedId, setExpandedId] = useState(null)
  const [items, setItems] = useState({})
  const [itemsLoading, setItemsLoading] = useState({})
  const [invalModal, setInvalModal] = useState(null) // { dte, modo: 'invalidar'|'revertir' }

  // Permisos: solo ejecutivo/superadmin/admin pueden invalidar
  ...
  // Invalidar / revertir DTE (proveedor refacturó sin emitir NC)
  const handleInvalidar = async ({ dte, modo, motivo }) => {
    try {
      ...
      setInvalModal(null)
      alert(modo === 'revertir' ? '✅ DTE re-activado' : '⊘ DTE invalidado (excluido del P&L y KPIs)')
    } catch (e) {
                ...
                <React.Fragment key={r.id}>
                  ...
                  >
                    ...
                    <td style={sTd}>
                      ...
                      {r.invalidado && <span style={{ ...sChip(C.red, 'rgba(230,57,70,0.20)'), marginLeft: 4, fontWeight: 700 }} title={r.motivo_invalidacion || ''}>INVALIDADO</span>}
                    </td>
