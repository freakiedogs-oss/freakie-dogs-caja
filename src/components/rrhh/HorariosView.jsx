...
export default function HorariosView({ user }) {
  ...
  const [horarios, setHorarios] = useState({}); // key: `${usuario_id}-${dia}`
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(null); // key siendo guardado
  const [editCell, setEditCell] = useState(null); // { usuario_id, dia }
  ...
  const loadHorarios = useCallback(async () => {
    ...
    const { data } = await db.from('horarios_empleados')
      .select('*')
      .eq('sucursal', sucursalSel)
      .eq('semana_inicio', semana);
    const map = {};
    (data || []).forEach(h => { map[`${h.usuario_id}-${h.dia_semana}`] = h; });
    setHorarios(map);
    setLoading(false);
  }, [sucursalSel, semana]);
  ...
  const guardarCelda = async (usuario_id, dia, data) => {
    ...
    try {
      ...
      if (data === null) {
        // Borrar
        if (existing?.id) {
          await db.from('horarios_empleados').delete().eq('id', existing.id);
        }
        setHorarios(prev => { const n = { ...prev }; delete n[key]; return n; });
      } else {
        const payload = {
          usuario_id, sucursal: sucursalSel, semana_inicio: semana,
          dia_semana: dia, creado_por: user.id, ...data,
        };
        if (existing?.id) {
          await db.from('horarios_empleados').update(payload).eq('id', existing.id);
          setHorarios(prev => ({ ...prev, [key]: { ...existing, ...payload } }));
        } else {
          const { data: inserted } = await db.from('horarios_empleados').insert(payload).select().single();
          setHorarios(prev => ({ ...prev, [key]: inserted }));
        }
      }
      showToast('Guardado');
    } catch (e) {
  ...
  const copiarSemanaAnterior = async () => {
    ...
    const { data } = await db.from('horarios_empleados')
      .select('*').eq('sucursal', sucursalSel).eq('semana_inicio', semanaAnterior);
    ...
    const inserts = data.map(h => ({
      ...
      turno: h.turno, notas: h.notas, creado_por: user.id,
    }));
    await db.from('horarios_empleados').upsert(inserts, { onConflict: 'usuario_id,semana_inicio,dia_semana' });
    await loadHorarios();
  ...
  return (
    <div style={{ padding: '16px 12px' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 13, color: c.textDim, marginTop: 2 }}>Asignación semanal de turnos</div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          <button onClick={() => setSemana(addDays(semana, -7))}
            style={{ padding: '7px 12px', borderRadius: 8, background: c.input, color: c.text, border: `1px solid ${c.border}`, cursor: 'pointer', fontSize: 16 }}>‹</button>
          <div style={{ fontSize: 13, color: c.text, fontWeight: 600, whiteSpace: 'nowrap', minWidth: 130, textAlign: 'center' }}>
            {fmtSemana(semana)}
          </div>
          <button onClick={() => setSemana(addDays(semana, 7))}
            style={{ padding: '7px 12px', borderRadius: 8, background: c.input, color: c.text, border: `1px solid ${c.border}`, cursor: 'pointer', fontSize: 16 }}>›</button>
          <button onClick={() => setSemana(getLunes(new Date()))}
            style={{ padding: '7px 10px', borderRadius: 8, background: c.input, color: c.textDim, border: `1px solid ${c.border}`, cursor: 'pointer', fontSize: 12 }}>Hoy</button>
        </div>

        {canEdit(user) && (
          <button onClick={copiarSemanaAnterior}
            style={{ padding: '7px 12px', borderRadius: 8, background: c.input, color: c.textDim, border: `1px solid ${c.border}`, cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>
            📋 Copiar semana ant.
          </button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ borderCollapse: 'collapse', minWidth: 600 }}>
            <thead>
              <tr>
                <th style={{ padding: '8px 12px', textAlign: 'left', fontSize: 12, color: c.textDim, fontWeight: 700, borderBottom: `2px solid ${c.border}`, minWidth: 140, background: c.bg, position: 'sticky', left: 0, zIndex: 2 }}>
                  Empleado
                </th>
                {DIAS.map((d, i) => {
                  ...
                  return (
                    <th key={i} style={{ padding: '6px 8px', textAlign: 'center', fontSize: 12, color: esHoy ? c.red : c.textDim, fontWeight: 700, borderBottom: `2px solid ${c.border}`, minWidth: 90, background: c.bg }}>
                      <div>{d}</div>
                      <div style={{ fontSize: 10, fontWeight: 400, color: c.textOff }}>
                        {new Date(fechaDia + 'T12:00:00').toLocaleDateString('es-SV', { day: '2-digit', month: 'short' })}
                      </div>
                    </th>
                  );
                })}
              ...
              {empleados.map((emp, ei) => (
                <tr key={emp.id} style={{ background: ei % 2 === 0 ? c.bg : '#141414' }}>
                  ...
                  {DIAS.map((_, di) => {
                    ...
                    const isEditing = editCell?.usuario_id === emp.id && editCell?.dia === di;
                    ...
                    return (
                      <td key={di} style={{ padding: 4, borderBottom: `1px solid ${c.border}`, verticalAlign: 'top' }}>
                        {isEditing ? (
                          ...
                          <div
                            onClick={() => canEdit(user) && setEditCell({ usuario_id: emp.id, dia: di })}
                            style={{
                              minHeight: 52, borderRadius: 6, padding: '4px 6px', cursor: canEdit(user) ? 'pointer' : 'default',
                              background: turnoInfo ? turnoInfo.bg : 'transparent',
                              border: `1px solid ${turnoInfo ? turnoInfo.color + '44' : c.border}`,
                              transition: '0.1s', opacity: isSaving ? 0.5 : 1,
                            }}
                          >
                            {h ? (
                              <>
      ...
      {toast && (
        <div style={{
          ...
          padding: '10px 20px', borderRadius: 8, fontSize: 13, fontWeight: 600, zIndex: 1000,
          background: toast.ok ? 'rgba(74,222,128,0.15)' : 'rgba(230,57,70,0.15)',
          color: toast.ok ? c.green : c.red, border: `1px solid ${toast.ok ? c.greenDark : c.red}`,
...
function CeldaEditor({ initial, onSave, onCancel }) {
  ...
  // Auto-rellenar horas según turno
  const handleTurno = (t) => {
    setTurno(t);
  ...
  return (
    <div style={{ background: c.card, border: `1px solid ${c.red}`, borderRadius: 8, padding: 8, minWidth: 130 }}
      onClick={e => e.stopPropagation()}>
      {/* Turno selector */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginBottom: 6 }}>
        {Object.entries(TURNOS).map(([k, v]) => (
          <button key={k} onClick={() => handleTurno(k)} style={{
            padding: '2px 7px', borderRadius: 20, fontSize: 10, fontWeight: 700, border: 'none', cursor: 'pointer',
            background: turno === k ? v.bg : c.input, color: turno === k ? v.color : c.textDim,
          }}>{v.label}</button>
        ))}
      </div>
      ...
      {turno !== 'descanso' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, marginBottom: 6 }}>
          <div>
            <div style={{ fontSize: 10, color: c.textDim, marginBottom: 2 }}>Entrada</div>
            <input type="time" value={horaInicio} onChange={e => setHoraInicio(e.target.value)} style={small} />
          </div>
          <div>
            <div style={{ fontSize: 10, color: c.textDim, marginBottom: 2 }}>Salida</div>
            <input type="time" value={horaFin} onChange={e => setHoraFin(e.target.value)} style={small} />
          </div>
        </div>
      )}

      <input value={notas} onChange={e => setNotas(e.target.value)}
        placeholder="Nota (opcional)" style={{ ...small, marginBottom: 6 }} />

      <div style={{ display: 'flex', gap: 4 }}>
        <button onClick={() => onSave({ turno: turno || null, hora_inicio: horaInicio || null, hora_fin: horaFin || null, notas: notas || null })}
