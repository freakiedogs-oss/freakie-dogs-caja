...
export default function TabJuego({ show = () => {} }) {
  ...
  const cargar = () => db.rpc('torre_ganadores_juego', { p_token: token, p_dias: 30 })
    .then(({ data, error }) => { if (error) setErr(error.message); else setLista(data || []); });
  ...
  if (!lista) return <div style={{ color: c.dim, padding: 20 }}>Cargando…</div>;
    ...
    <div>
      ...
      {lista.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {lista.map(g => (
            <div key={g.score_id} style={{ background: c.card, border: `1px solid ${c.border}`,
                                           borderLeft: `3px solid ${g.premiado ? c.green : c.yellow}`,
                                           borderRadius: 12, padding: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: c.dim }}>{fechaBonita(g.fecha)}</span>
              ...
              <div style={{ fontSize: 12, color: c.dim, marginTop: 6, lineHeight: 1.6 }}>
                ...
                {g.partidas_del_dia} partida(s) ese día
              </div>
