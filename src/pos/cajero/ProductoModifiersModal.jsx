...
// Modal que se abre al tocar un producto del menú POS.
...
//
// Estructura esperada de `grupos` (viene del fetch en POSMain):
//   [{ id, nombre, tipo: 'single'|'multiple', obligatorio, min_selecciones, max_selecciones,
...
export default function ProductoModifiersModal({
  ...
  grupos = [],    // grupos de modificadores del producto
  onClose,
  onConfirm,
}) {
  const [tab, setTab]           = useState('modificadores') // 'modificadores' | 'producto'
  const [qty, setQty]           = useState(1)
  const [nota, setNota]         = useState('')
  // selecciones: { [grupoId]: [opcionId, opcionId, ...] }
  const [selecciones, setSelecciones] = useState({})
  // dropdown abierto (uno a la vez por default)
  const [openGroupId, setOpenGroupId] = useState(null)

  // Al montar: si hay grupos con min_selecciones > 0 no cumplido, abrir el primero obligatorio
  useEffect(() => {
  ...
  // Si no hay grupos, saltar directo a "Producto seleccionado"
  useEffect(() => {
  ...
  // Precio adicional total por modificadores × qty
  ...
  // Validación: todos los grupos obligatorios cumplen su min_selecciones
  const validacion = useMemo(() => {
    const errores = []
    for (const g of grupos) {
      const nSel = (selecciones[g.id] || []).length
      const min = g.min_selecciones || 0
      if (g.obligatorio && nSel < min) {
        errores.push(`Faltan opciones en "${g.nombre}" (mín. ${min})`)
      }
    }
    return errores
  }, [selecciones, grupos])
  ...
  const handleConfirm = () => {
    ...
    // Estructura para guardar en pos_cuenta_items.modificadores (jsonb)
    // Un array plano de {grupo, nombre, precio_extra} — fácil de renderizar en ticket
  ...
  return (
    <div className="pos-modal-overlay" onClick={onClose}>
      ...
      >
              ...
              {grupos.map(grupo => {
                ...
                return (
                  <div key={grupo.id} style={{ border: '1px solid #2a2a32', borderRadius: 8, background: '#22222c', overflow: 'hidden' }}>
                    {/* Label del grupo - AMARILLO BOLD UPPERCASE (consistencia con KDS) */}
                    ...
                    >
                      ...
                      <span style={{ color: '#8b8997', fontSize: 12 }}>{abierto ? '▲' : '▼'}</span>
                    </button>
                      ...
                      <div style={{ borderTop: '1px solid #2a2a32', maxHeight: 260, overflowY: 'auto', background: '#1a1a22' }}>
                        {grupo.opciones.map(opt => {
                          ...
                          return (
                            ...
                            >
                              <input
                                type={grupo.tipo === 'single' ? 'radio' : 'checkbox'}
                                checked={selected}
          ...
          {tab === 'producto' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  width: 40, height: 40, borderRadius: 8, background: '#22222c',
              ...
              <div>
                ...
                <div style={{
                  ...
                  border: '1px solid #2a2a32', borderRadius: 12, padding: 4, background: '#22222c',
                }}>
                  <button
                    onClick={() => setQty(q => Math.max(1, q - 1))}
                    style={{
                      background: '#1a1a22', border: '1px solid #2a2a32', width: 60, height: 42,
                      borderRadius: 8, fontSize: 22, cursor: 'pointer', color: '#e5e7eb',
                    }}
                  >−</button>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  style={{
                    width: '100%', border: '1px solid #2a2a32', borderRadius: 8,
                    padding: 12, fontSize: 14, resize: 'vertical', fontFamily: 'inherit',
                    background: '#22222c', color: '#e5e7eb',
                  }}
                />
              </div>
            </div>
        ...
        {/* Footer con subtotal + botón agregar */}
        <div style={{ padding: '12px 16px', borderTop: '1px solid #2a2a32', background: '#1e1e26' }}>
          {precioModificadoresUnit > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#8b8997', marginBottom: 6 }}>
              <span>Producto ${precioProductoUnit.toFixed(2)} + modificadores ${precioModificadoresUnit.toFixed(2)} × {qty}</span>
