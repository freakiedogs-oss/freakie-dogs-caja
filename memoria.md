...
## 24-Ago-2026 — Rediseño visual del Kardex (ítem 7 del plan): una sola paleta, un ícono por concepto, InfoTips en los 9 tabs
- **Paleta única `src/components/almacen/kardexUi.jsx`:** la referencia de estilo es el tab Fugas (tarjetas con borde de color a la izquierda, pills, tablas limpias). Exporta `K` (superficies/texto/acentos), `tint()`, `pill()`, `tituloSec/th/td` y `Tarjeta`; `DiferenciasTab` ahora importa de ahí (borró sus copias locales) y `KardexView` reemplazó **todos** sus hex sueltos (había ~5 verdes, 3 azules y 3 grises distintos según el tab). Los tokens shadcn de `global.css` se quedan para clases utilitarias (`bg-background`, etc.); los acentos viven en `K` porque los estilos son inline y Fugas ya definía los valores buenos.
- **Iconografía única:** un emoji por concepto en tabs, TIPOS y MOV_TIPOS: MP 🥩 (era 🧾, que parecía factura), SP 🧪, PT 🍔, IN 🧰 (era 📦, chocaba con el tab Inventario). Colores de movimiento consistentes con Fugas: conteo naranja, merma rojo, ajuste azul; entradas verde, venta morado, traslado/consumo gris.
- **InfoTip en cada tab** (encabezado `TabHeader` con título + ⓘ): qué muestra, de dónde sale el dato y qué significa, escrito para el dueño. Para los tabs que viven en componentes propios (Fugas, Conteo, Menú, Recetas, Costeo) el encabezado se pone desde KardexView — no se tocaron los hijos.
- **Inventario con existencias:** tocar un producto expande su stock por sucursal (SELECT solo-lectura a `inventario` + embed `sucursales`, cargado al expandir para no bajar todo el inventario al iPhone). Negativo en rojo con nota de qué significa; total al pie.
- **Historial con QUIÉN y filtro por tipo:** embed `usuarios_erp(nombre)` (FK verificada; ¡NO empleados!) — "sin usuario" sale en naranja (movimientos automáticos, ej. venta del POS). Pills de filtro por tipo de movimiento (server-side `.eq('tipo',…)`). Filas con borde izquierdo del color del tipo.
- **Borrado el editor muerto de recetas** de KardexView (estados + escrituras a la tabla obsoleta `recetas_lineas`): nunca se renderizaba — el tab muestra `RecetasView`. Verificado con grep que nada más lo llamaba; también se quitó el `fetchRecetas()` fantasma al cambiar de tab.
- **Sin tocar:** RPCs, lógica de datos, pills Fugas reales/Consumo interno del PR #304 (solo cambiaron a referenciar `K.*` con los mismos valores).
