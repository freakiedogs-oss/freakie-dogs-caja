...
| 34 | Conteo de Críticos | OPS | Pestaña dentro de `ConsumoVentaDashboard` (21-Sep-2026, `ConteoCriticosTab.jsx` + lógica pura `criticosConteo.js`). Espejo de la hoja diaria de Saúl, por sucursal y día. **Él digita:** CID (paquetes + unidades, en las 15 filas), Se pidió (paquetes completos), Desc. AM/PM (**bodega de sucursal → cocina, control interno; NO es la venta y no entra en el cálculo**), TPS Final (paquetes en bodega) y En Línea (unidades sueltas abiertas). **El sistema pone:** Venta del día (kardex `venta`), propone Se pidió desde los traslados y **arrastra el CID del cierre real de ayer** (TPS+Línea del día anterior; si no se digita, el cálculo lo usa — marcado con ↩). `teórico = CID + Se pidió − Venta del día`; `real = TPS Final + En Línea`; `dif = real − teórico` ≡ *venta − consumo físico* — **negativo = se fue producto que ninguna venta descontó**. **Modo Semana** (sólo lectura, 10 semanas lunes–domingo): `fn_criticos_semana` cierra la ecuación **día por día** y suma sólo los días completos; corta el arrastre si falta un día en medio. **KPIs valorizados** contra `v_fd_costo_insumo`: descuadre neto y % sobre venta, faltante y sobrante **por separado**, venta en críticos, contadas X/15 y peor descuadre por plata. `null` ≠ `0` de punta a punta. RPCs `fn_criticos_hoja` / `fn_criticos_semana` / `fn_criticos_guardar` / `fn_criticos_reabrir`. Hereda permisos de `consumo-venta`. Pruebas: `scripts/test-criticos.mjs` (129) y `scripts/test-criticos-render.mjs` (56). |
