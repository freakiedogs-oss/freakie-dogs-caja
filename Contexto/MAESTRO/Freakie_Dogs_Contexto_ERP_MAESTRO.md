...
| 64 | criticos_items | Filas de la hoja de **Productos Críticos** de Saúl (`Criticos_FD_FORMATO.xlsx`), 15 ítems en 4 categorías. Define nombre, presentación, unidad de conteo, `factor` (unidades de stock por empaque), `fraccionado` + `unidad_suelta`/`factor_suelta` y `nota_config` (dónde el Excel y el catálogo NO coinciden — sale en pantalla). No es un flag en `catalogo_productos` porque una fila puede mapear a varios productos. |
| 65 | criticos_item_productos | Mapeo fila→producto(s) del catálogo con `factor_a_item` (unidades de stock de la FILA por 1 unidad del producto). Existe por "Pan Tradicional HD": el insumo cambió el 25-ago-2026 (unidades → bolsa de 10, `factor_a_item` 0.1 y 1), y una hoja anterior sólo cuadra si los dos suman. |
| 66 | criticos_conteo | Cabecera de la hoja: UNIQUE (sucursal_id, fecha), `estado` abierto/cerrado, notas, quién la cargó. |
| 67 | criticos_conteo_items | Lo digitado: `cid_enteros`+`cid_sueltas` (apertura), `pedido_enteros` (paquetes completos), `descarga_am`/`descarga_pm` (**bodega de sucursal → cocina, control interno de Saúl; no es la venta**), `tps_enteros` (paquetes enteros en bodega al cerrar) y `linea_sueltas` (unidades sueltas de los paquetes abiertos, en cocina). **NULL ≠ 0** — null es "no lo contó" y 0 es "contó y no había"; `fn_criticos_guardar` BORRA la fila si viene toda en null en vez de guardar ceros. `factor`/`factor_suelta` se **congelan** al digitar para que una hoja vieja no cambie de significado si el catálogo cambia. La venta del día NO se guarda: se recalcula en cada lectura. |
...
| 34 | Conteo de Críticos | OPS | Pestaña dentro de `ConsumoVentaDashboard` (`ConteoCriticosTab.jsx` + lógica pura `criticosConteo.js`). Espejo de la hoja diaria de Saúl, por sucursal y día. **Dos clases (23-sep):** *porcionado* (8 — piezas contables, cuadran EXACTO, "En Línea" son piezas sueltas) y *peso* (7 — bolsa/bandeja, la abierta se pesa y "En Línea" es la FRACCIÓN del empaque, 0.40 de bolsa, con 5% de margen). `criticos_items.clase` + `tolerancia_pct` (override por ítem). **Él digita:** Se pidió (paquetes), Desc. AM/PM (**bodega→cocina, control interno; no entra en el cálculo**), TPS Final (paquetes en bodega) y En Línea. **El CID NO se digita**: es el cierre de ayer y se arrastra (↩); sólo se abre el primer día de una sucursal, sin cierre previo (✎). **El sistema pone:** Venta del día (kardex) y propone Se pidió desde los traslados. `teórico = CID + Se pidió − Venta`; `real = TPS + En Línea`; `dif = real − teórico` ≡ *venta − consumo físico*; **negativo = se fue producto que ninguna venta descontó**. Semáforo binario (cuadra / descuadre), con medio grano de redondeo para los porcionados. **Modo Semana** (10 semanas lunes–domingo, sólo lectura): cierra la ecuación día por día y corta el arrastre si falta un día. **KPIs valorizados** contra `v_fd_costo_insumo`: descuadre, faltante y sobrante por separado, venta en críticos, contadas X/15, peor descuadre por plata. `null` ≠ `0` de punta a punta. RPCs `fn_criticos_hoja` / `fn_criticos_semana` / `fn_criticos_guardar` / `fn_criticos_reabrir`. Hereda permisos de `consumo-venta`. Pruebas: `scripts/test-criticos.mjs` (142) y `scripts/test-criticos-render.mjs` (57). |
