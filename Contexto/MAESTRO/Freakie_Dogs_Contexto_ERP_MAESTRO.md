...
| 67 | criticos_conteo_items | Lo digitado: `cid_*`, `pedido_*`, `tps_*`, `linea_*` en enteros + sueltas. **NULL ≠ 0** — null es "no lo contó" y 0 es "contó y no había"; `fn_criticos_guardar` BORRA la fila si viene toda en null en vez de guardar ceros. `factor`/`factor_suelta` se **congelan** al digitar para que una hoja vieja no cambie de significado si el catálogo cambia. Las descargas NO se guardan: se recalculan en cada lectura. |
...
| 34 | Conteo de Críticos | OPS | Pestaña dentro de `ConsumoVentaDashboard` (21-Sep-2026, `ConteoCriticosTab.jsx` + lógica pura `criticosConteo.js`). Espejo de la hoja diaria de Saúl: Saúl digita CID / TPS Final / En Línea por sucursal y día, y el sistema pone **Descargas AM/PM** (kardex `venta` partido por turno de caja, o por hora fija) y propone **"Se pidió"** desde los traslados recibidos. `teórico = CID + Se pidió − Descargas`; `real = TPS Final + En Línea`; `dif = real − teórico`, que es idénticamente *descargas del sistema − consumo físico* — **negativo = se fue producto que ninguna venta descontó**. RPCs `fn_criticos_hoja` / `fn_criticos_guardar` / `fn_criticos_reabrir`. Semáforo con piso absoluto de 2 unidades sueltas + 2%/5%. **El corte por turno sólo funciona en Venecia (S004)**: es la única que abre un 2º turno; Lourdes tiene 2 cajas ambas `numero_turno`=1 (misma jornada en paralelo). Donde no hay 2º turno todo cae en AM y la pantalla lo avisa. Sin nav_key propia: hereda `consumo-venta` (admin, ejecutivo, superadmin). Pruebas: `scripts/test-criticos.mjs` (80) y `scripts/test-criticos-render.mjs` (render jsdom). |
