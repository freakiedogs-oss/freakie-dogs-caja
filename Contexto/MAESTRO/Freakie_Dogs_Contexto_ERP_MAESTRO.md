...
| 34 | Conteo de Críticos | OPS | Pestaña dentro de `ConsumoVentaDashboard` (21-Sep-2026, `ConteoCriticosTab.jsx` + lógica pura `criticosConteo.js`). Espejo de la hoja diaria de Saúl, por sucursal y día. **Él digita:** CID (paquetes + sueltas), Se pidió (paquetes completos), Desc. AM/PM (**movimiento bodega de sucursal → cocina, control interno; NO es la venta y no entra en el cálculo**), TPS Final (paquetes enteros en bodega) y En Línea (unidades sueltas de los paquetes abiertos). **El sistema pone:** Venta del día (kardex `venta`, un solo número) y propone Se pidió desde los traslados recibidos. `teórico = CID + Se pidió − Venta del día`; `real = TPS Final + En Línea`; `dif = real − teórico` ≡ *venta del día − consumo físico* — **negativo = se fue producto que ninguna venta descontó**. `null` ≠ `0` de punta a punta: una celda vacía deja la fila sin veredicto y el servidor borra la fila que llega toda en null. RPCs `fn_criticos_hoja` / `fn_criticos_guardar` / `fn_criticos_reabrir`. Semáforo con piso absoluto de 2 unidades sueltas + 2%/5%. Sin nav_key propia: hereda `consumo-venta` (admin, ejecutivo, superadmin). Pruebas: `scripts/test-criticos.mjs` (88) y `scripts/test-criticos-render.mjs` (render jsdom, 32). |
